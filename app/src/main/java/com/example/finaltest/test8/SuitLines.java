...
//                }
    ...
    private void drawBackground(Canvas canvas) {
        ...
        //
        canvas.drawRect(yArea.right, yArea.bottom - yArea.height() / (countOfY - 1) * 4, linesArea.right, yArea.bottom - yArea.height() / (countOfY - 1) * 2, paint);
        //恢复画笔颜色
        paint.setColor(Color.YELLOW);
        //画格子
        for (int i = 0; i < 10; i++) {
            float y = yArea.bottom - yArea.height() / (countOfY - 1) * i;
            float x = xArea.left + xArea.width() / (maxOfVisible - 1) * i;
            if (i == maxOfVisible / 2) {
                paint.setColor(Color.RED);
            }
            canvas.drawLine(x, yArea.top, x, linesArea.bottom, paint);
            paint.setColor(Color.YELLOW);
            canvas.drawLine(yArea.right, y, linesArea.right, y, paint);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = suitEdge[0]; i <= suitEdge[1]; i++) {
            for (int j = 0; j < datas.size(); j++) {
                ...
                canvas.drawCircle(current.getXY().x, current.getXY().y, 5, new Paint(Paint.ANTI_ALIAS_FLAG));
            }
        }
        ...
        drawY(canvas);
        for (int j = 0; j < datas.size(); j++) {
            Unit midUnit = datas.get(j).get(maxOfVisible/2);
            ToastUtil.singleShow(midUnit.getValue()+" / "+midUnit.getExtX());
        }
    }
    ...
    private void onTip(/*float upX, float upY*/) {
//        upX -= offset;
...
//        bak.offset(-offset, 0);
        ...
        float abs = Math.abs(offset % realBetween);
        Log.d(TAG, "1onTip: " + abs + "  /   " + offset + "   /   " + realBetween);
        int index = -1;
        int d = (int) Math.abs(offset / realBetween);
        if (abs < 10 && abs > 0) {
            index = maxOfVisible / 2 + d;
        } else if ((abs > realBetween - 10 && abs < realBetween)) {
            index = maxOfVisible / 2 + 1 + d;
        }
        Log.d(TAG, "2onTip: " + index);
        if (index != -1) {
            for (int i = 0; i < datas.size(); i++) {
                Unit unit = datas.get(i).get(index);
                ToastUtil.singleShow(unit.getValue() + "   /  " + unit.getExtX());
                Log.d(TAG, "3onTip: " + unit.getExtX());
            }
        }
    }
    ...
    private void RedLine(float moveX) {
        float x = xArea.left + xArea.width() / 6 * 3;
        ...
        for (int i = 0; i < datas.get(0).size(); i++) {
            for (int j = 0; j < datas.size(); j++) {
                datas.get(j).get(i).setXY(new PointF(linesArea.left + realBetween * i,
                    linesArea.top + linesArea.height() * (1 - (datas.get(j).get(i).getValue() - 80) / 100)));
